...
class Simulation:
    ...
    def run(self, generations, render=False):
        ...
        for gen in range(generations):
            ...
            fitness_function = partial(run_agent, env=copy.copy(self.env), 
                                       epochs=self.epochs)
...
def main():
    ...
    import pickle
    # save best weights for future uses
    file = open(directory + env_name + '-bests-st-' + str(seed) +'.plt','rb')
    bests = pickle.load(file)
    file.close()
    
    print("Replay best agents")
    for i in range(len(bests)):
        print(f"***Generation {i}, expected reward {bests[i].fitness}")
        real_reward = run_agent(bests[i], env, epochs=1, render_test=True)
...
def run_simulation(seed):
    ...
    # save stats to file
    np.savetxt(directory + env_name + '-reward-st-' + str(seed)+'.txt', list(zip(best_fitness, 
                                                            avg_fitness)), \
               fmt='%.18g', delimiter='\t', header="Fitness: Best, Avg")
